    ...
    <div className="mx-auto max-w-7xl px-4 py-8 pb-20">
      ...
      <div className="grid gap-8 md:grid-cols-[1.8fr_1fr]">
        <section className="space-y-4 rounded-xl bg-[#131313] p-4 md:p-6">
          ...
          <input className="surface-high w-full rounded px-3 py-3 text-sm" placeholder="Full name" value={name} onChange={(e) => setName(e.target.value)} />
          ...
          <input className="surface-high w-full rounded px-3 py-3 text-sm" placeholder="Street address" value={street} onChange={(e) => setStreet(e.target.value)} />
          ...
          <div className="flex flex-wrap gap-2 text-sm">
            <button
              type="button"
              className={`btn-secondary px-3 py-2 ${method === "standard" ? "!border-[#c8f135] !text-[#c8f135]" : ""}`}
            ...
            <button
              type="button"
              className={`btn-secondary px-3 py-2 ${method === "express" ? "!border-[#c8f135] !text-[#c8f135]" : ""}`}
          ...
          <div className="flex flex-wrap gap-2 text-sm">
            {["qpay", "socialpay", "card", "cod"].map((p) => (
              <button
                key={p}
                type="button"
                className={`btn-secondary px-3 py-2 uppercase ${payment === p ? "!border-[#c8f135] !text-[#c8f135]" : ""}`}
        ...
        <aside className="sticky top-24 h-fit rounded-xl bg-[#141414] p-4">
          ...
          <p className="mt-2 text-sm text-zinc-300">Subtotal: {formatMnt(subtotal)}</p>
          {discount > 0 && <p className="text-sm text-green-700">SAVE10: -{formatMnt(discount)}</p>}
          <p className="text-sm text-zinc-300">Delivery: {formatMnt(deliveryFee)}</p>
          <p className="mt-2 border-t border-zinc-700 pt-2 font-mono font-semibold text-[#c8f135]">Total: {formatMnt(total)}</p>
