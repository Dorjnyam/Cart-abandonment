...
export default function CartPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8 pb-20">
      <h1 className="mb-4 font-brand text-3xl font-black uppercase">Shopping cart</h1>
      <div className="ml-auto max-w-3xl rounded-2xl bg-black/60 p-4 md:p-6">
        <div className="space-y-3">
          {items.map((item) => (
            <div key={`${item.id}-${item.size}-${item.color}`} className="kick-card flex items-center justify-between gap-3 p-3">
              <div>
                <p className="font-medium">{item.name}</p>
                <p className="text-xs text-zinc-500">EU {item.size} · {item.color} · qty {item.qty}</p>
                <p className="mt-1 font-mono text-xs text-[#c8f135]">{formatMnt(item.price * item.qty)}</p>
        ...
        <aside className="mt-4 space-y-3 rounded-xl bg-[#141414] p-4">
          ...
          <div className="text-sm">Subtotal: {formatMnt(subtotal)}</div>
          {discount > 0 && <div className="text-sm text-green-700">Discount: -{formatMnt(discount)}</div>}
          <div className="text-sm text-zinc-500">Delivery: {formatMnt(5000)}</div>
          <div className="h-2 overflow-hidden rounded-full bg-zinc-800">
            <div className="h-full w-2/3 bg-[#c8f135]" />
          </div>
          <p className="text-xs text-zinc-400">{formatMnt(30000)} more for free shipping</p>
          <div className="border-t pt-2 font-semibold">Total: {formatMnt(total)}</div>
          ...
          <div className="kick-card p-3 text-xs text-zinc-300">Upsell: Add sneaker care kit +{formatMnt(15000)}</div>
        </aside>
      </div>
    </div>
  );
